...
- `score` 为 `1-10` 分，`note` 承接“本次复盘”。`attachments` 为数组，单项固定为 `{ id, type, fileID, thumbFileID?, duration?, size?, createdAt }`；附件只支持查看和删除，不支持编辑、剪辑、标注或独立媒体库。
...
| action | 说明 |
| --- | --- |
| `material.seed` | 已停用代码内 seed，调用时提示改为手动导入 `release-support/mock-data/improv_materials.json`。 |
...
4. 从 `release-support/mock-data/` 手动导入集合数据：

- `release-support/mock-data/improv_materials.json`
- `release-support/mock-data/improv_user_material_states.sample.json`
- `release-support/mock-data/improv_inspirations.sample.json`
- `release-support/mock-data/improv_rehearsals.sample.json`
- `release-support/mock-data/improv_method_cards.sample.json`
- `release-support/mock-data/improv_practice_records.sample.json`
