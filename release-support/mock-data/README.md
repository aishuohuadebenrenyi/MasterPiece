# release-support/mock-data 导入说明

更新时间：2026-06-24

`release-support/mock-data/` 存放可手动导入 CloudBase 的演示数据，不参与小程序构建，也不参与云函数部署。当前项目采用重建策略，素材体系以 `Material` 为上位对象，不兼容旧 `Game` 集合数据。
...
素材字段按四个维度管理：`type` 使用固定 8 类且必填，`abilities` 使用固定训练能力，`scenes` 使用固定场景，`tags` 承载可自由扩展的细粒度特征。2026-06-24 已将原先混在 `abilities` 中的反应力、热身、双人等词汇迁移到 `tags`。如果 CloudBase 中已有旧版公共素材，应清空 `ownerOpenId: "system"` 的旧记录后重新导入，避免重复或继续显示旧字段语义。
...
5. 批量替换 `release-support/mock-data/*.sample.json` 里的 `__REPLACE_WITH_OPENID__`。
...
- `docs/data-api.md`
- `release-support/wechat-cloudbase-app/database.md`
