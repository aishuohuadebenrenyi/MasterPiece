...
function attachmentMetaPills(attachments = []) {
  ...
  return []
    .concat(imageCount ? [`${imageCount} 张照片`] : [])
    .concat(videoCount ? [`${videoCount} 个视频`] : [])
    .concat(audioCount ? [`${audioCount} 段录音`] : [])
}

function uniqueMeta(meta = []) {
  return Array.from(new Set((Array.isArray(meta) ? meta : []).filter(Boolean)))
}
...
function normalizePendingItem(item, sourceType) {
  ...
  const attachmentMeta = attachmentMetaPills(item.attachments)
  return Object.assign({}, item, {
    ...
    title,
    desc,
    meta: uniqueMeta([label].concat(attachmentMeta).concat(meta.filter((entry) => entry !== label))),
    filteredMeta: uniqueMeta(attachmentMeta.concat(meta.filter((entry) => entry && entry !== label)))
  })
}
...
function buildMineViewData({ sediments = [], inspirations = [], rehearsals = [], practiceRecords = [], playedCount = 0, layoutStyle = '', profile, methodFilter = 'all', inspirationFilter = 'all', discardedPendingKeys = [], pendingIntentMarks = [] } = {}) {
  ...
  const mappedInspirations = inspirations.map((item) => attachSummaryCardView(Object.assign({}, item, {
    ...
    type: item.type || '灵感',
    filteredMeta: uniqueMeta(attachmentMetaPills(item.attachments).concat(filterItemMeta(Object.assign({}, item, {
      type: item.type || '灵感'
    }))))
  })))
  const discarded = new Set(discardedPendingKeys)
...
Page({
  ...
  openPracticeRecords() {
    wx.navigateTo({ url: '/pages/record-overview/index?filter=practice' })
  },
  ...
  async chooseProfileAvatar() {
    try {
      ...
      if (!message.includes('cancel')) toast('选择头像失败，请重试')
    }
  },
  ...
  async saveProfile() {
    ...
    try {
      ...
      if (response.code !== 0 || !response.item) {
        toast(response.message || '保存失败，请重试')
