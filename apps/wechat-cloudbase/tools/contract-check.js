...
assert(
  backend.includes("[COLLECTIONS.inspirations]: ['id', 'title', 'desc', 'meta', 'linkedMaterialId', 'linkedMaterialTitle', 'linkedRehearsalId', 'linkedRehearsalTitle', 'sourceType', 'sourceId', 'sourceTitle', 'attachments']"),
  'inspiration.create must allow attachments'
)
assert(
  backend.includes("[COLLECTIONS.inspirations]: ['title', 'desc', 'meta', 'linkedMaterialId', 'linkedMaterialTitle', 'linkedRehearsalId', 'linkedRehearsalTitle', 'sourceType', 'sourceId', 'sourceTitle', 'attachments']"),
  'inspiration.update must allow attachments'
)
assert(
  backend.includes('collectionName === COLLECTIONS.practiceRecords || collectionName === COLLECTIONS.inspirations'),
  'inspiration attachments must receive media security checks'
)

const recordPage = read('frontend/pages/record/index.ts')
assert(recordPage.includes('async function cleanupUploadedAttachments'), 'record page must clean up failed inspiration uploads')
assert(recordPage.includes('await cleanupUploadedAttachments(attachments)'), 'record page must clean up attachments after an inspiration write failure')
...
const releaseRunbook = fs.readFileSync(path.join(root, '..', '..', 'docs', 'operations', 'release-runbook.md'), 'utf8')
