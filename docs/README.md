...
| [brand-guidelines.md](brand-guidelines.md) | 品牌与上架规范，包含核心色系、Logo 含义及 AppStore 上架文案与截图规范。 |
...
当 `docs/data-architecture.md` 与 `wechat-cloudbase-app/database.md` 出现差异时，以工程内 `database.md` 为实现事实源，并同步修订 `docs/data-architecture.md`。
...
- 数据集合、字段、云函数 action 变化时，先更新 `wechat-cloudbase-app/database.md`，再同步 `data-architecture.md`。
