# 微信小程序与 CloudBase 技术架构

更新时间：2026-07-17

## 1. 文档目的

本文档沉淀 `apps/wechat-cloudbase` 的实际技术架构。跨平台关系见 [总体技术架构](overview.md)，iOS 实现见 [iOS / iPadOS 架构](ios.md)。
...
`apps/wechat-cloudbase/project.config.json`：
...
全局 `frontend/app.wxss` 只承载页面骨架、卡片、按钮、筛选 chips、表单输入、状态面板、图标语义和 fixed action bar 等跨页面语义基线；具体页面的局部组合结构不应继续堆进全局样式。按钮统一使用 `.primary-btn`、`.ghost-btn`、`.small-btn`、`.mini-action-btn` 表达动作层级：主提交、次主动作、卡片/列表低强调动作和页面工具栏紧凑动作分别复用对应类，页面 WXSS 只补布局尺寸与定位，不重新发明颜色、边框、阴影和圆角。卡片、弹层和表单内部的补充按钮、确认小动作和标签新增入口必须按文字内容自适应宽度，只保留必要左右 padding；除页面级提交、底部 fixed action 或明确双按钮动作区外，不得用 `width: 100%`、`flex: 1` 或过大的固定 `min-width` 把局部按钮拉成长胶囊。tag/chip/meta pill 行属于全局 UI 语义：默认左对齐、固定 12rpx 紧凑间距、过多自然换行；页面不得用 `space-between`、等宽 `flex: 1`、默认横向滚动或截断溢出来分散标签，确需横向浏览时必须显式使用横向滚动容器。组件内部若用原生 `button` 承载 chip，必须在组件自身的类选择器中清除原生 margin、宽度和 flex 行为，不能只依赖全局 `button` reset。图标统一先挂 `.icon-text`，再按用途补 `.icon-back`、`.icon-close`、`.icon-search`、`.icon-random`、`.icon-favorite` 等语义类；页面只处理位置和尺寸，不单独重写图标风格。非 fixed 场景中的内联保存操作组应由页面局部样式控制，不与 `form-actions` / `fixed-actions` 共用同一套固定底栏语义。
...
生产前端的 `bottom-sheet`、状态面板和子页面头部是覆盖率最高的基础设施。组件覆盖率只用于衡量跨页面基础设施，业务专用组件应按适用场景覆盖评价。页面内新增卡片必须先归入导航、内容、参考、操作、反馈或容器六类角色，不能以单页私有色值、阴影和圆角建立同层级新卡种。
...
详细集合和 action 见 [数据契约](data-contract.md) 与 [CloudBase 工程事实源](../../apps/wechat-cloudbase/docs/database.md)。
