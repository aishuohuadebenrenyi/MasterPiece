...
- 主 Tab：发现、记录、我的。
- 已声明页面：发现、记录、我的、游戏详情、灵感记录、游戏实践反馈、排练记录、排练复盘、团队排练记录、游戏记录，共 10 个页面。
...
- `wechat-cloudbase-app/project.config.json` 中的 `appid` 已替换为真实微信小程序 AppID。
...
- `game.seed` 仍保留 action，但只提示改为手动导入 `mock_data/improv_games.json`。
...
当前产品定位是“找游戏 + 快记录 + 可沉淀”，不是单纯游戏百科。

已经收敛的关键交互结论：

- 首次无游戏库时，发现页聚焦一个“添加游戏 / 马上开玩”的大卡片，不保留完整筛选和抽卡骨架。
- 有游戏库但筛选无结果时，说明条件过窄，并提供清空或放宽筛选动作。
